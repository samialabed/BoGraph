...
from autorocks.experiments.rocksdb.cfgs import available_optimizer, available_params
from autorocks.logging_util import ENV_RUNNER_LOGGER

LOG = logging.getLogger(ENV_RUNNER_LOGGER)

if __name__ == "__main__":
    parser = ArgumentParser(description="Run RocksDB case-study")
    parser.add_argument(
        "--execution_plan",
        type=ExecutionPlan,
    ...
    parser.add_argument(
        "--bench",
        type=dbbench_benchmarks.DBBenchTasks,
        help="Name of the benchmark to use",
        choices=list(dbbench_benchmarks.DBBenchTasks),
        required=True,
    )
    ...
    bench_cfg = cli_options.bench.get_plan()
    env_cfg = RocksDBEnvConfig(bench_cfg=bench_cfg)
